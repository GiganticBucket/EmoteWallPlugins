...
function makeBarrelRollEmotes(n: number): EmoteData[] {
    let result: EmoteData[] = [];
    for (let i = 0; i < n; i++) {
...
async function LaunchBarrelRolls(barrelRolls: EmoteData[]) {
    ...
    for (let i = 0; i < numToFire; i++) {
        ...
        let configurers = new EmoteConfigurerList(
            ...
            new InitialVelocityConfigurer(angle, fuzzNumber(550, 50)),
        ...
        let behaviors = new EmoteBehaviorList(
            ...
            new BounceOffWallsBehavior([false, false, false, false]));
