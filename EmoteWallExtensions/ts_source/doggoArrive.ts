...
async function FunStuff(doggoData: EmoteData): Promise<void> {
    for (let i = 0; i < 10; i++) {
        let emote = new OverlayEmote(
            doggoData,
            new OverlayEmoteState(5),
            new EmoteConfigurerList(
                new DoggoConfigurer(),
                new InitialPositionConfigurer(window.innerWidth / 2, window.innerHeight / 2),
                new InitialVelocityConfigurer(0, 0),
                new ExplicitStartingDimensionsConfigurer(50, 50)),
            new EmoteBehaviorList(new DoggoBehavior(), doggoOpacityBehavior));
...
class DoggoConfigurer implements IOverlayEmoteConfigurer {
    name: "DoggoConfigurer";
    configure(startingOverlayEmote: OverlayEmote): void {
        startingOverlayEmote.state.image.style.zIndex = "-1";
    }
}

class DoggoBehavior implements IOverlayEmoteBehavior {
    name: "doggoBehavior";
