registerPlugin({
    ...
    testButtons: [
        {
            buttonText: "Win",
            callback: () => simulateMessage("GiganticBucket You got catJAM | catJAM | catJAM and won 2000 Donut Holes! DiscoShell DiscoShell DiscoShell You stayed at rank #9.", "giganticbucket", "giganticbucketbot")
        },
        {
            buttonText: "Loss",
            callback: () => simulateMessage("GiganticBucket You got catJAM | catJAM | ratJAM and lost your 100 Donut Holes. DiscoShell You stayed at rank #9.", "giganticbucket", "giganticbucketbot")
        }
    ]
});
async function showSlotsResult(message) {
    let overlayEmotes = [];
    let won = message.text.includes(" and won ");
    let individualSlotEmoteSpace = won ? 200 : 100;
    for (let i = 0; i < 3; i++) {
        let individualSlotEmote = message.emotes[i];
        overlayEmotes.push(new OverlayEmote(individualSlotEmote, new OverlayEmoteState(6), new EmoteConfigurerList(...[{
...
//# sourceMappingURL=SlotsResult.js.map